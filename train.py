...
def train(args):
    ...
    if args.lr_scheduler:
        schedulerG = torch.optim.lr_scheduler.CosineAnnealingLR(optimizerG, args.init_num_iterations + args.num_iterations * (args.num_phase-1), eta_min=args.eta_min)
    ...
    if args.lr_scheduler:
        schedulerD = torch.optim.lr_scheduler.CosineAnnealingLR(optimizerD, args.init_num_iterations + args.num_iterations * (args.num_phase-1), eta_min=args.eta_min)
    ...
    g2 = Loss_inv(args.phi2, init_iteration, args.loss_scheduler, args.num_phase, 10, 2, args.alpha_min, args.alpha_max, 0, None)
...
if __name__ == '__main__':
    ...
    # (ADD) phase and iters
    parser.add_argument('--phi2', type=str, default='kl')
    parser.add_argument('--init_num_iterations', type=int, default=20000)
    parser.add_argument('--num_iterations', type=int, default=400, help='the number of iterations')
    parser.add_argument('--init_h', type=float, default=1)
    parser.add_argument('--h', type=float, default=1, help='proportion of the cost c')
    parser.add_argument('--num_phase', type=int, default=1000)
    parser.add_argument('--loss_scheduler', type=str, default=None, choices=[None, 'steplr', 'linear', 'cosine'])
    parser.add_argument('--alpha_min', type=float, default=1, help='Min alpha')
    parser.add_argument('--alpha_max', type=float, default=5, help='Max alpha')
